...
pub fn assert_stable_engagement(
    ...
    for (man, _preferences) in men_preferences {
        ...
        let women_who_rejected_this_man =
            get_women_who_rejected_this_man(*man, *woman_engaged_to.unwrap(), &men_preferences);

        for woman in women_who_rejected_this_man {
            let current_man = get_currently_engaged_man_for_woman(woman, &engaged_man_woman);
            assert_woman_does_not_prefer_this_man_over_her_current_man(
                woman,
                current_man,
                *man,
                women_preferences,
...
fn get_women_who_rejected_this_man(
    man: u32,
    woman_engaged_to: u32,
    men_preferences: &HashMap<u32, Vec<u32>>,
) -> Vec<u32> {
...
fn assert_woman_does_not_prefer_this_man_over_her_current_man(
    woman: u32,
    current_man: u32,
    man: u32,
    women_preferences: &HashMap<u32, Vec<u32>>,
